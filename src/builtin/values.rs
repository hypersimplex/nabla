...
pub fn init_builtin_values(env: &mut EnvVVarToTyScheme, ns: &mut TyVarNameSupply) {
    init_vvar_ty_scheme_builtin_unary_plus(env, ns);
    init_vvar_ty_scheme_builtin_binary_plus(env, ns);
    init_vvar_ty_scheme_builtin_unary_minus(env, ns);
    init_vvar_ty_scheme_builtin_unary_not(env, ns);
    init_vvar_ty_scheme_builtin_binary_minus(env, ns);
    init_vvar_ty_scheme_builtin_binary_mul(env, ns);
    init_vvar_ty_scheme_builtin_binary_div(env, ns);
    init_vvar_ty_scheme_builtin_binary_lt(env, ns);
    init_vvar_ty_scheme_builtin_binary_le(env, ns);
    init_vvar_ty_scheme_builtin_binary_gt(env, ns);
    init_vvar_ty_scheme_builtin_binary_ge(env, ns);
    init_vvar_ty_scheme_builtin_binary_eq(env, ns);
    init_vvar_ty_scheme_builtin_logical_and(env, ns);
    init_vvar_ty_scheme_builtin_logical_or(env, ns);
    init_vvar_ty_scheme_builtin_match_fail(env, ns);
}

// maps builtin value-level variable unary plus to type scheme ([tv_numeric] (tv_numeric -> tv_numeric))
pub fn init_vvar_ty_scheme_builtin_unary_plus(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    let tvar_numeric = ns.generate();
    env_vvar_to_ty_scheme.0.insert(
        VVar::Named(VVarName {
...
pub fn init_vvar_ty_scheme_builtin_binary_plus(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    let tvar_numeric = ns.generate();
...
pub fn init_vvar_ty_scheme_builtin_unary_minus(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    let tvar_numeric = ns.generate();
...
pub fn init_vvar_ty_scheme_builtin_unary_not(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    env_vvar_to_ty_scheme.0.insert(
...
pub fn init_vvar_ty_scheme_builtin_binary_minus(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    let tvar_numeric = ns.generate();
...
pub fn init_vvar_ty_scheme_builtin_binary_mul(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    let tvar_numeric = ns.generate();
...
pub fn init_vvar_ty_scheme_builtin_binary_div(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    let tvar_numeric = ns.generate();
...
pub fn init_vvar_ty_scheme_builtin_binary_lt(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
...
pub fn init_vvar_ty_scheme_builtin_binary_le(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    let tvar_any = ns.generate();
...
pub fn init_vvar_ty_scheme_builtin_binary_gt(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    let tvar_any = ns.generate();
...
pub fn init_vvar_ty_scheme_builtin_binary_ge(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    let tvar_any = ns.generate();
...
pub fn init_vvar_ty_scheme_builtin_binary_eq(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    let tvar_any = ns.generate();
...
// note: mk_ty_bool resolves to the concrete Bool ADT
pub fn init_vvar_ty_scheme_builtin_logical_and(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    env_vvar_to_ty_scheme.0.insert(
...
// note: mk_ty_bool resolves to the concrete Bool ADT
pub fn init_vvar_ty_scheme_builtin_logical_or(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    env_vvar_to_ty_scheme.0.insert(
...
pub fn init_vvar_ty_scheme_builtin_match_fail(
    env_vvar_to_ty_scheme: &mut EnvVVarToTyScheme,
    ns: &mut TyVarNameSupply,
) {
    let tvar_any = ns.generate();
